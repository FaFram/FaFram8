...
/**
 * Testing killing containers on Windows machines.
 *
 * @author : Roman Jakubco (rjakubco@redhat.com)
 */
@Slf4j
public class RemoteWindowsKillingTest {
	...
	private Container childRoot = ChildContainer.builder().name("child-root").parent(root).build();

	@Rule
	public Fafram fafram = new Fafram().provider(new OpenStackProvisionProvider()).containers(root, ssh, childRoot);
	...
	public void testWindowsCluster() throws Exception {
		childRoot.kill();
		assertTrue(root.executeNodeCommand(
				"WMIC PROCESS get Processid,Commandline /format:list  | grep -E 'karaf.*org.apache.karaf.main.Main' | grep " + childRoot.getName()).isEmpty());

		ssh.kill();
		assertTrue(ssh.executeNodeCommand("WMIC PROCESS get Processid,Commandline /format:list  | grep -E 'karaf.*org.apache.karaf.main.Main'").isEmpty());

		root.kill();
		assertTrue(root.executeNodeCommand("WMIC PROCESS get Processid,Commandline /format:list  | grep -E 'karaf.*org.apache.karaf.main.Main'").isEmpty());
